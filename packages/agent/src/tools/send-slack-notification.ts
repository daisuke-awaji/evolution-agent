...
export const sendSlackNotification = tool({
  ...
  description: 'Send a Slack notification via an incoming webhook. The webhook URL is fetched from AWS Secrets Manager.',
  inputSchema: z.object({
    webhookSecretName: z.string().describe('Secrets Manager secret name containing the Slack webhook URL'),
  ...
  callback: async ({ webhookSecretName, message, blocks }) => {
    ...
    const webhookUrl =
      secretResponse.SecretString?.trim() ?? '';
